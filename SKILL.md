...
3. **数据架构保证** - 三级缓存架构（L1实时/L2技术指标/L3基本面），每晚自动更新，确保数据新鲜

## 数据架构（V3.2.2 重要修复）

### 三级缓存体系

| 级别 | 数据类型 | 缓存时间 | 更新时机 | 存储位置 |
|-----|---------|---------|---------|---------|
| **L1** | 实时价格 | 5分钟 | 盘中实时 | 内存 + 文件 |
| **L2** | 技术指标 | 1天 | 每晚23:00 | 文件缓存 |
| **L3** | 基本面 | 7天 | 每晚23:00 | 文件缓存 |

### 统一数据服务 (core/unified_data_service.py)
**新增核心模块**，整合所有数据源：
- 多源自动切换：腾讯 → 新浪 → 东财
- 智能缓存管理：内存缓存 + 文件缓存
- 统一接口：`get_data_service().get_stock_data(codes)`

### 夜间缓存更新 (scripts/nightly_cache_update.py)
**每晚23:00自动运行**：
- 更新技术指标缓存（MA、20日高低点等）
- 更新基本面缓存（PE/PB/ROE等）
- 数据源：东方财富K线 + 财务数据

### 代码健康检查 (scripts/code_health_review.py)
**每晚3:00自动运行**：
- 检查当日数据获取成功率
- 检查缓存命中率
- 检查ETF修复等关键补丁是否存在
...
### 关键修复（2026-03-13）
- ✅ **ETF基金识别**：5开头代码正确识别为`sh`（上交所）
- ✅ **多数据源兜底**：腾讯失败→新浪→东财，自动切换
- ✅ **缓存过期处理**：三级缓存独立管理，避免数据过期
...
| 版本 | 日期 | 主要更新 |
|------|------|----------|
| **V3.2.2** | **2026-03-13** | **架构修复：三级缓存、统一数据服务、夜间自动更新、代码健康检查** |
...
| 时间 | 任务 | 脚本 | 说明 |
|------|------|------|------|
| 23:00 | 夜间缓存更新 | `scripts/nightly_cache_update.py` | 更新技术指标、基本面缓存 |
| 03:00 | 代码健康检查 | `scripts/code_health_review.py` | 检查数据获取成功率、缓存健康度 |
| 09:30-15:00 | 盘中监控 | `scripts/intraday_monitor_integrated.py` | 每10分钟运行 |

**Crontab配置**:
```bash
# 每晚23:00更新缓存
0 23 * * * cd /path/to/skill && python3 scripts/nightly_cache_update.py >> logs/nightly.log 2>&1

# 每晚3:00健康检查
0 3 * * * cd /path/to/skill && python3 scripts/code_health_review.py >> logs/health.log 2>&1
```
